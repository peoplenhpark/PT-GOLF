...
> **최종 갱신:** 2026-09-12 · **현재 상태:** seed `v33` · SW `ptgolf-v48` · 자산 `?v=48`/`ASSET_VER='48'` · `main` = `origin/main` 동기화됨.
...
## 0-1. 2026-09-12 전체 운동 2컷 + 3D

- 사용자 최신 요청: **케이블 푸시다운을 표준으로 나머지 동작에도 2컷+3D 삽입**. 적용 범위는 현재 등록된 47개(PT43 + 골프4).
- 바로 열기: https://peoplenhpark.github.io/PT-GOLF/?v=48#exercise/pt_deadbug. 상세는 #exercise/<id>로 직접 열 수 있다.
- 순서: **2컷 → 입체로 자세 보기 버튼 → 움직임·느낌 → 기존 핵심 체크·잊지 말 것·공통 원칙 → 내 메모**. 기존 스텝·스펙·준비 블록도 유지한다.
- 2컷 확대 안내, 3D 부제·소개 문구는 추가하지 않는다. 브라우저 기본 핀치 확대와 3D 내부 드래그·핀치 조작을 유지한다.
- seed v33과 각 운동 갱신일은 변경하지 않는다. 시각 자료는 js/exercise-media.js로 분리했고 localStorage 메모·즐겨찾기·개별 수정은 그대로 읽는다.
- 새 2컷 46세트는 docs/images/guides/<id>-start.webp / -end.webp. 푸시다운은 기존 samples/pushdown-3d 이미지·전용 뷰어 유지.
- 나머지 46개 3D는 media/3d/viewer.html?exercise=<id>. poses.js가 운동별 자세·기구, viewer.js가 인체·근육 위치·회전·재생을 담당한다.
- 열 때만 iframe을 만들고 접거나 상세를 나가면 제거한다. iframe 높이 메시지는 출처·발신 프레임을 검증한다.
- 데드버그 약40%, SLR 반대 무릎 세우기, 풀오버 무릎 당겨 고정, 티밸런스 지지·뒷다리 펴기, 벤치 이상근 스트레칭 등 개인 코칭 변형을 반영했다.
- **이전 '골프 이미지 없음'과 '뒤꿈치 받침 스쿼트 이미지 불필요' 결정은 이번 전체 운동 적용 요청으로 변경됐다.** 골프의 6개 핵심 키워드 스트립은 그대로 보존한다.
- 전체 프롬프트·경로·검증·오프라인 동작: docs/visuals/README.md. 자동 검증은 tests/visual-media.cjs, 최종 결과는 docs/visuals/verification.json.
- 새 큰 이미지는 방문 시 캐시한다. 셸·3D 공통 코드·라이브러리는 선캐시한다. 한번 방문해 불러온 운동은 오프라인 재접속을 검증한다.
- 자산 버전48: 기존5곳(index CSS/Store/App, App ASSET_VER, SW CACHE)에 더해 index의 exercise-media.js 및 공통 뷰어 내부 버전도 함께 갱신한다.
- 이전 이미지 작업에서 남은 js/store.js의 SVG 경로 이행 코드는 원문/개인 메모를 바꾸지 않는 보조 처리다. 현재 시각 자료는 registry로 연결하므로 그 조건에 의존하지 않는다.
...
- **`image` 필드:** 운동별 시각 registry가 있으면 2컷+3D가 우선이고, 없는 사용자 추가 운동에서는 기존 `.ex-figure`로 렌더. 경로는 루트 기준 상대경로.
- **편집 시 image 보존됨:** 편집 폼엔 image 입력이 없지만 `Store.upsert`가 기존 객체를 스프레드 병합하므로 유지됨.
- ⭐**`prep` 필드**(2026-09-04 신설): 「준비 자세」 배열. 상세에서 **스펙 아래·그림 위**에 `🧩 준비 자세` 블록으로 렌더되고, 없으면 표시되지 않는다. 사용자 요청(*"준비동작, 운동중 핵심포인트, 반복횟수 중량에 유의"*)으로 **준비 / 실행 / 수치**를 분리한 것 — `prep`=준비 자세, `cues`=**운동 중 핵심**(블록 제목도 변경됨), `spec`=**중량 · 횟수 × 세트**. ★**현재 8동작 보유**(9/4·9/10 세션분). ⭐**다음 세션부터, 반영하는 운동마다 그 자리에서 `prep`을 채워 점진 완성할 것**(한 번에 43개를 자동 분류하면 오분류 위험이 커서 의도적으로 점진 적용). `Store.search`도 `prep`을 훑는다.
- **`focus` 필드**(2026-08-06 신설, **47동작 전부 보유**): `{muscle, move, feel}` — 상세 **2컷·3D 아래**에 `.focus-box`로 «🎯 타깃 근육 / 움직임 / 느낌» 카드를 렌더. 기존 cues·원칙에 흩어진 트레이너 코칭을 **3줄로 압축한 요약 레이어**(중복 아님 — 먼저 읽는 개념). `Store.search`도 focus를 훑으므로 **「중둔근」「대퇴사두」 등 근육명으로 검색** 가능. 새 동작 추가 시 focus도 같이 작성할 것.
- **`steps` 필드**(2026-07-30 신설): 있으면 `renderDetail`이 상세 **최상단**(스펙 위)에 `.steps-flow`로 **키워드 흐름**(`›` 구분)을 렌더. 현재 골프 4종만 사용(4종 동일 문구) — `준비 골반접기 › ①백스윙(어깨 높이 유지) › ②체중이동+손 내리기 › ③지연(수직 낙하까지) › ④임팩트(몸으로·면타격) › ⑤팔 펴기·중심 확인`. **스크롤 없이 `flex-wrap:wrap`으로 한 화면에 전부 노출**(가로 스크롤 방식은 폐기). ⚠️ 골프는 **시퀀스 일러스트를 만들었다가 사용자 요청으로 제거**(`d4d7bb5`→`0963670`)하고 이 키워드 스트립으로 대체함. 이번 전체 운동 2컷·3D 요청으로 이미지가 추가됐지만 키워드 스트립은 유지한다(파일 `28/29_golf_steps_*.svg`는 미참조로 잔존 — 이 때문에 신규 그림 번호는 **30번부터** 시작함).
- 현재: `version 33`, exercises **47**, principles **10**. PT 칩 순서 = 등 / 팔 / 가슴 / 어깨 / 하체 / 하체 · 머신 / 일일 수행 / 하체 재활 · 일일수행2 / **코어 · 밸런스** (골프 = 드라이버·아이언).
...
- **`어깨` 카테고리**(2026-09-01 신설): 페이스 풀 · 사이드 레터럴 레이즈 + 전용 원칙 `pr_pt_shoulder`. ⭐트레이너 커리큘럼 = 하체 → 등 → 어깨 → **가슴(9/10 완료)** — ★**전 부위 1회전 완료**. 예고대로 이후엔 가짓수를 늘리지 않고 **부위별로 돌린다**. ⇒ 앞으로 녹취는 **신규보다 기존 동작의 디테일 보강** 위주가 될 가능성이 높다.
...
- **이미지 커버리지**: 현재47개 모두 2컷+3D. 새 운동은 최신 코칭을 기준으로 같은 차콜 배경·은색 인체/회색 운동복·붉은 근육 표시로 제작하고 js/exercise-media.js에 등록한다. 기존 SVG는 이전 원본으로 보관한다.
...
| `c62dacb` | **9/4 세션 + 구조 개선**. ⭐**사용자 요청으로 `prep`(준비 자세) 필드 신설** — 상세에서 「🧩 준비 자세」 / 「✅ 운동 중 핵심」(구 자세 체크리스트) / `spec`(중량·횟수×세트)로 **3분할**. 카테고리 **`추천운동` → `코어 · 밸런스`** 개명. 신규 ①**고블릿 스쿼트(중량 앞으로)**(`하체`, **7kg 10~15회×3** — 앞으로 쏠리는 힘을 **등으로 버티고** 나머지는 고관절에. **중심은 앞·엉덩이만 뒤로**, 발바닥에서 엉덩이를 밀어 올린다) ②**싱글 레그 데드리프트(외발)**(`코어 · 밸런스`, 좌우 10회×3 — ⭐**체중 먼저 앞으로 → 그다음 다리**, 몸을 앞뒤로 **길게 뽑고**, **균형 되는 범위까지만**, 올라오면 **바짝** 서기. 지지대는 중심 잡기용). 보강 ③**레그레이즈**(준비=무릎 최대한 펴기·손바닥 바닥·팔 펴기 / ⭐**템포 교정** — 늘릴 때 천천히, **수축은 더 빠르게**. 느리기만 하면 힘만 걸려 있다 / **15~20회×3세트**) ④**로테이션 런지**(준비=체중 먼저 앞·**앞꿈치:뒤꿈치 6:4~7:3** / 뒤로 뺄 때 **「내 발로 서는 느낌」** / **뒤꿈치 밟고 발 전체로** 일어나기 / 팔로 밀지 말고 **반대쪽 골반을 튼다**). 원칙 `pr_pt_rep`에 **기본 3세트**·템포 추가. 도구 **`tools/jsbal.py`**(node 부재 환경용 JS 괄호 검사) 추가. 일러스트 `41`·`42` 신규. seed **v31**(45동작), SW **v45**. |
| `dbb154f` | **9/10 세션 — 가슴 파트(커리큘럼 1회전 완료)**. 신규 ①**체스트 프레스(머신)**(`가슴`, **15kg** — 「**큰 벽을 넓게 받아준다**」. ⭐**미는 순서 = 흉추(가슴 내밀기) → 등 가운데 → 어깨·가슴·팔**. 준비 = 다리 걸어 **흔들림 0** · **견착**(척골·요골이 함께 밀리게, 손목만 힘 X = 총 개머리판 비유) · 허리 뒤 손 하나. 천천히 들어오고 **멈추는 위치를 알 것**) ②**펙덱 플라이**(`가슴` — ⭐**팔로 시작하지 말 것, 가슴이 먼저 움직이고 팔이 따라온다**. 준비 = **전면 어깨로 팔을 밀어놓고** 팔꿈치–몸 사이 손 하나. **열 때도 계속 내밀며 잡고**, 늘어난 걸 느낀 뒤 모은다. **무겁게 할 필요 없음** — 축이 바깥이라 같은 무게도 무겁게 느껴진다). 재정비 ③**레그 익스텐션**(준비 신설 = ⭐**패드를 발등이 아니라 「발목」에** · **무릎은 항상 정면** · 골반 먼저 / 템포 = **속도를 조금 빨리·위에서 멈추지 않기** / **양발·한 발 병행**. ★골반은 완벽히 못 잡는 게 정상 — 「**올릴 때 골반까지 힘이 올라오느냐**」만 보면 된다) ④**와이드 스쿼트 → 「내전근」 프레임으로 재작성·개명**(무릎은 항상 **발끝 방향** · **내전근이 브레이크를 잡으며** 내려간다 · **딱 걸리는 지점**에서 올라오기. 8/18 봉 버전은 변형 1줄로 보존). 원칙 = `pr_pt_chest`(미는 순서·무게보다 해부학적 움직임·**안쪽 가슴만 따로 못 키운다**) · `pr_pt_rep`(**주 2회 권장**·정자세가 무게보다 앞선다) · `pr_golf_common`(⭐**백스윙 = 축발 안쪽으로 「넣는 힘」 + 왼발 「벌리는 힘」** — 열고 돌리면 둔근이 안 걸리고 **눌러 잡고 돌리는 순간** 걸린다). 일러스트 `43`·`44` 신규, `33` 교체. seed **v33**(47동작), SW **v46**. |
| `-` | **고블릿 스쿼트 명칭 확정**(2026-09-04 사용자 확인). 녹취에 「오블리스쿼티」로 들렸던 종목명이 **고블릿 스쿼트**로 확정 — 불확실 경고 리마인더 제거. seed **v32**(콘텐츠만, 자산 bump 없음). |
...
- **이미지 = 2컷 해부학적 3D 스타일 + 회전형 모델.** 사용자의 전체 적용 요청으로 예전 SVG 유지/실사 보류 방침이 변경됐다. 기본은 내장 image_gen으로 생성하고 WebP로 최적화한다. 기존 SVG는 삭제하지 않는다. 자세한 현재 기준은 §0-1 및 docs/visuals/README.md.
...
4. **시각 자료 개별 교정** — 운동 후 사용자 피드백에 따라 해당 2컷/3D만 수정한다. 전체 새 디자인 적용은 완료.
