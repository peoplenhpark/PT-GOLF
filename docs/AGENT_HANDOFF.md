...
> **최종 갱신:** 2026-09-12 · **현재 상태:** seed `v33` · SW `ptgolf-v47` · 자산 `?v=47`/`ASSET_VER='47'` · `main` = `origin/main` 동기화됨.
...
## 0-1. 2026-09-12 케이블 푸시다운 완성본

- 바로 열기: https://peoplenhpark.github.io/PT-GOLF/?v=47#exercise/pt_pushdown. 운동 상세는 #exercise/<id>로 직접 열 수 있다.
- 현재 순서: **2컷 → 입체로 자세 보기 버튼 → 움직임·느낌 → 기존 핵심 체크·잊지 말 것·공통 원칙 → 내 메모**.
- 사용자 요청으로 2컷 손가락 확대 안내, 3D 부제/소개 문구를 제거했다. 확대 기능은 그대로 유지한다.
- pt_pushdown의 모든 원문과 갱신일(2026-08-21)은 변경하지 않았다. seed v33 유지. 기존 localStorage 메모·즐겨찾기·개별 수정도 그대로 사용한다.
- samples/pushdown-3d/index.html은 실제 상세로 연결하고, viewer.html은 3D 전용 iframe이다. 클릭 시 로드, 접기/상세 종료 시 제거. 프레임 높이 메시지는 출처와 발신 프레임을 검증한다.
- 2컷과 3D 자산을 재사용하며 케이블 푸시다운에만 새 배치를 적용했다. 나머지 운동 일러스트 일괄 교체는 별도 미완료 작업이다. 이전 작업의 js/store.js 이미지 경로 이행 변경은 미커밋 상태로 남아 있다.
- 모바일·원문 대조·기존 메모/즐겨찾기/큐 보존·메모 저장/재접속·3D 실행/회전/접기 검증 완료.
- 자산 버전 5곳을 47로 맞췄고, 새 미디어와 뷰어를 SW 캐시에 추가했다. 상세: samples/pushdown-3d/README.md.
...
- **`focus` 필드**(2026-08-06 신설, **47동작 전부 보유**): `{muscle, move, feel}` — 상세 **최상단**에 `.focus-box`로 «🎯 타깃 근육 / 움직임 / 느낌» 카드를 렌더. 기존 cues·원칙에 흩어진 트레이너 코칭을 **3줄로 압축한 요약 레이어**(중복 아님 — 먼저 읽는 개념). `Store.search`도 focus를 훑으므로 **「중둔근」「대퇴사두」 등 근육명으로 검색** 가능. 새 동작 추가 시 focus도 같이 작성할 것.
