...
(async()=>{
 ...
 try{
  ...
  for(const e of seed.exercises){
   await page.goto(base+'?v=50#exercise/'+e.id);await page.waitForSelector('.guide-pair img');
   ...
   assert.deepEqual(order,e.id==='pt_pushdown'?['focus-box','pushdown-guide','exercise-3d']:['exercise-guide','exercise-3d','focus-box']);
