...
assert.equal(c.videos.length,31);assert.equal(new Set(c.videos.map(v=>v.id)).size,31);
...
assert.equal(c.videoGroups.length,4);assert.equal(new Set(grouped).size,31);assert.equal(grouped.length,31);
assert.deepEqual(Array.from(c.videoGroups,g=>g.videoIds.length),[2,3,12,14]);
...
ctx.window.GolfHub.render({golfTab:'videos'},api);check();assert.equal((app.innerHTML.match(/class="g-video-card(?: [^"]*)?"/g)||[]).length,31);
...
console.log('PASS: 31 originals, 4 groups, no golf 3D UI/cache, 47 PT models preserved, 7 old URL redirects, no storage writes.');
